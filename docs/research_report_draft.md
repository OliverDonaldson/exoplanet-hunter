...
**Inference and discovery.** Branch 4 (planned) will score the 6,200 held-out TOI/Kepler Planet Candidates in `data/labels/candidates.parquet` (4,570 TESS + 1,630 Kepler), build a six-panel vetting figure (phase-folded global, local, odd/even, BLS periodogram, centroid drift, model probability + MC-Dropout uncertainty), and submit the top high-confidence candidates for manual review against ExoFOP / TFOP records. This is the discovery pathway: any candidates surviving manual review against existing dispositions would be eligible for community follow-up. Cross-validation against the RAVEN catalog of Lafarga et al. (2026) — particularly their ~1,000 vetted TESS-SPOC FFI candidates that are not yet TOI or CTOI — provides an additional independent check on our top-ranked candidates beyond ExoFOP and the NEA Planetary Systems table.
