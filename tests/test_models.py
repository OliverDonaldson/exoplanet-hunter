...
def test_cnn_dualview_builds_and_runs(cnn_cfg) -> None:       # noqa: ANN001
...
def test_cnn_dualview_trains_one_step(cnn_cfg) -> None:       # noqa: ANN001
    from exoplanet_hunter.models import build_cnn_dualview
    ...
    history = model.fit(
        ...
        y, epochs=1, batch_size=4, verbose=0,
    )
    assert "loss" in history.history


def test_cnn_with_aux_features(cnn_cfg) -> None:              # noqa: ANN001
    from exoplanet_hunter.models import build_cnn_dualview
    ...
    out = model(
        {"global_view": g, "local_view": l, "aux_features": a}
    ).numpy()
